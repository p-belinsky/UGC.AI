...
import {GhostButton, PrimaryButton} from "./Buttons.tsx";
...
const ProjectCard = ({gen, setGenerations, forCommunity = false}: {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            setGenerations((generations)=>generations.filter(generation=>generation.id !== id))
    ...
    const togglePublish = async (projectId: string) => {
        try {
            ...
            setGenerations((generations)=>generations.map((generation)=>generation.id === projectId ? {...generation, isPublished: data.isPublished} : generation))
        ...
        <div key={gen.id} className='mb-4 break-inside-avoid'>
            <div className='bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group'>
                <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
                    ...
                    {gen.generatedVideo && (
                        <video src={gen.generatedVideo} playsInline loop muted className='absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:opacity-100 opacity-0' onMouseEnter={(e)=>e.currentTarget.play()} onMouseLeave={(e)=>e.currentTarget.pause()}/>
                    )}
                        ...
                        <div
                            ...
                            <div className='flex flex-col items-end w-32 text-sm'>
                                <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}>
                                    ...
                                    <button className='w-full flex gap-2 items-center px-4 py-2 hover:bg-red-950/10 text-red-400 cursor-pointer' onClick={()=> handleDelete(gen.id)}>
