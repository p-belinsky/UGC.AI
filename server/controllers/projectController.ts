...
export const createProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if(tempProjectId!){
            await prisma.project.update({where: {id: tempProjectId}, data: {isGenerating: false, error: message}})
        }
        if(isCreditDeducted){
...
export const createVideo = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await prisma.project.update({where: {id: projectId as string}, data: {isGenerating: true}})
        ...
        await prisma.project.update({where: {id: projectId as string}, data: {generatedVideo: uploadResult.secure_url, isGenerating: false}})
        ...
        const message = error instanceof Error ? error.message : 'An unknown error occurred';
            ...
            await prisma.project.update({where: {id: projectId as string, userId}, data: {isGenerating: false, error: message}})

        if(isCreditDeducted){
            await prisma.user.update({where: {id: userId}, data: {credits: {increment: 10}}})
        }
        res.status(500).json({message})
